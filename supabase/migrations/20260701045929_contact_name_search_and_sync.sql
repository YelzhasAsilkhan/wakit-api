...
CREATE OR REPLACE FUNCTION public.resolve_contact_name(p_organization_id uuid, p_address text)
 RETURNS text
 LANGUAGE sql
 STABLE
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.sync_conversation_names_for_address(
  p_organization_id uuid,
  p_address text,
  p_name text DEFAULT NULL::text,
  p_previous_name text DEFAULT NULL::text
)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.sync_conversation_names_on_contact_update()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.sync_conversation_names_on_address_link()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.link_contact_to_address(
  p_organization_id uuid,
  p_address text,
  p_name text
)
...
AFTER INSERT OR UPDATE OF contact_id ON public.contacts_addresses
FOR EACH ROW
EXECUTE FUNCTION public.sync_conversation_names_on_address_link();

-- Backfill saved contact names onto conversations for search.
update public.conversations c
set name = public.resolve_contact_name(c.organization_id, c.contact_address)
where c.contact_address is not null
  and (
    c.name is null
    or c.name = c.contact_address
  )
  and public.resolve_contact_name(c.organization_id, c.contact_address) is not null;
